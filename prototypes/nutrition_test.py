...
# === Deduplication file ===
seen_recipes_file = "uploaded_recipes.txt"
...
print(f"📡 Fetching up to {MAX_RECIPES} unique Chinese recipes...\n")

while collected < MAX_RECIPES and url:
    response = requests.get(url, params=params if collected == 0 else None)
    ...
    for hit in hits:
        ...
        for key, value in nutrients.items():
            if key == "ENERC_KCAL":
                continue  # Skip duplicate energy
...
# Save updated seen recipes
with open(seen_recipes_file, "w") as f:
    for name in sorted(seen_recipes):
        f.write(name + "\n")

print(f"\n🎉 Uploaded {collected} new recipe(s) with nutrition + metadata.")
